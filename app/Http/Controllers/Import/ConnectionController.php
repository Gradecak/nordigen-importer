...
class ConnectionController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($list as $item) {
            if (config('importer.customer_identifier', 'default_ff3_customer') === $item->identifier) {
                $hasCustomer = true;
                $identifier  = $item->id;
            }
        }
        ...
        if (false === $hasCustomer) {
            ...
            $request->identifier = config('importer.customer_identifier', 'default_ff3_customer');
